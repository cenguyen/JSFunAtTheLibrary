...
let buildMainCharacter = (name, age, pronouns) => {
...
let saveReview = (review, reviews) => {
  if (!reviews.includes(review)) {reviews.push(review);}
  return reviews;
}
...
let calculatePageCount = title => title.length * 20;
...
let writeBook = (title, character, genre) => {
  return {title, mainCharacter: character, pageCount: calculatePageCount(title), genre}
}

// function editBook(book) {
//   // takes 1 parameter: writeBook - book
//   // goal is to decrease page count to be 3/4 of original
//   // use dot notation to access book objs pageCount property
//   book.pageCount = book.pageCount * 0.75;
// }

let editBook = book => book.pageCount = book.pageCount * 0.75;
